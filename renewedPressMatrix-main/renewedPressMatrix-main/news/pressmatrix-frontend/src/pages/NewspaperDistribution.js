...
const NewspaperDistribution = () => {
  ...
  const fetchDistributions = async () => {
    try {
      ...
      if (user?.role === 'vendor') {
        // For vendors, we need to show distributions where they are the receiver
        const vendorDistributions = response.data.data.filter(dist => {
          console.log('Checking distribution:', {
            distribution_id: dist._id,
            receiver_id: dist.receiver_id?._id,
            user_id: user._id,
            status: dist.status,
            newspaper_name: dist.newspaper_name
          });
          return dist.receiver_id?._id === user._id && 
                 (dist.status === 'pending' || dist.status === 'distributed');
        });
        console.log('Filtered vendor distributions:', vendorDistributions);
        setDistributions(vendorDistributions);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (user.role === 'vendor') {
        ...
        const selectedDistribution = distributions.find(d => d._id === formData.distribution_id);
        if (!selectedDistribution) {
          throw new Error('Selected distribution not found');
        }
        await axios.patch(`http://localhost:5000/api/newspapers/updateUnsold/${selectedDistribution._id}`, distributionData);
    ...
    <Container maxWidth="lg">
      <Box sx={{ mt: 4, mb: 4 }}>
        <Typography variant="h4" component="h1" gutterBottom>
          {user.role === 'vendor' ? 'Newspaper Sales Update' : 
           user.role === 'area_distributor' ? 'Newspaper Receipt Update' : 
                ...
                >
                  ...
                  {distributions.map((distribution) => (
                    <MenuItem key={distribution._id} value={distribution._id}>
                      {distribution.newspaper_name} - Quantity: {distribution.quantity}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
